<template>
  <div v-if="checkRole()" class="main_big_customer">
    ...
    <!--      @success="getlist"-->
    <!--      @toggle="open_add = 1"-->
    <!--      @title_edit="title_edit($event)"-->
    <!--      @updatereview="title = 'Cập nhật đánh giá'"-->
    <!--      @backreview="title = `Đánh giá - ${dataItem.name}`"-->
    <!--      @addhistory="title = 'Thêm mới lịch sử'"-->
    <!--      @backhistory="title = `Lịch sử giao dịch - ${dataItem.name}`"-->
    <!--      @previewhistory="title = `Chi tiết giao dịch - #${$event.order_id_mask}`"-->
    <!--    ></edit>-->
...
.main_big_customer {
  ...
  .v-data-table--fixed-header .v-data-table__wrapper {
    max-height: calc(86vh - 220px) !important;
  }
}
</style>
