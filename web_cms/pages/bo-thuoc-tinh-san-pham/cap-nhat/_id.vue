    ...
    <v-form @submit.prevent="validate">
      <v-row class="px-6">
        <v-col v-if="embed" cols="12">
          ...
          >
            {{ $vuetify.lang.t('$vuetify.productAttrSet.updatePage.title') }}:
            {{ prodAttr.name }}
          </div>
        </v-col>
            ...
            >
              <v-row>
                ...
                <v-col cols="12" class="py-0">
                  ...
                  >
                    <!-- :readonly="
                        language.attrSetName && language.attrSetName.trim()
                          ? true
                          : false
                      " -->
                  </v-text-field>
                      ...
                      >
                        <template v-if="filteredCustomAttrs.length">
                          ...
                          >
                            ...
                            <td>
                              {{
                                prodAttr.attributeTypeName ||
                                prodAttr.attribute_type
                              }}
                            </td>
        ...
        <!-- Button Hủy và Lưu -->
        <v-footer app inset class="py-0" color="transparent">
          <v-row>
            <v-spacer></v-spacer>
...
export default {
  ...
  created() {
    if (!this.embed && !this.$route.query.ofSystem) {
  ...
  methods: {
    ...
    generateTabs() {
      ...
      this.getProdAttrSetInfo()
    },

    // Hàm lấy thông tin của bộ thuộc tính
    getProdAttrSetInfo() {
      this.hydrated = false
      this.$store
        .dispatch('productAttrSet/getProdAttrSetInfo', {
          attributeSetId: !this.embed
        ...
        .catch((error) => {
          this.$log.error('[getProdAttrSetInfo Error]', error)
        })
    },

    // Hàm hydrate dữ liệu từ raw data
    changeCurrentData() {
      if (Object.keys(this.rawData).length) {
        this.attributeLangList = this.attributeLangList.reduce(
          (acc, cur, idx) => {
            for (
              let index = 0;
              index < this.rawData.attributeSetLangs.length;
              index++
            ) {
              const language = this.rawData.attributeSetLangs[index]
              if (language.langId === cur.id) {
                cur.attrSetName = language.name
                cur.attrSetDes = language.subIntroduce
              }
            }
        ...
        this.customAttrs = [...this.rawData.customAttribute]
        this.defaultProdAttrList = [...this.rawData.sysAttribute]
        this.$store.commit(
          'app/setTitle',
          `${this.$vuetify.lang.t(
            '$vuetify.productAttrSet.updatePage.title'
          )}: ${this.rawData.attributeSetLangs[0].name}`
        )
        this.hydrated = true
      }
    },
    ...
    saveProdAttrSet() {
      ...
      const data = {
        attributeSet: {
          productCategoryId: 4,
          ofSystem: !this.embed ? this.$route.query.ofSystem : 2,
          id: this.rawData.id,
        },
      ...
      this.$store
        .dispatch('productAttrSet/updateProductAttrSet', data)
        .then((response) => {
          if (!response.error) {
            this.$showSuccess(
              this,
              'Cập nhật bộ thuộc tính sản phẩm thành công'
            )
            if (!this.embed) {
              this.$router.push('/bo-thuoc-tinh-san-pham')
            } else {
  ...
  head() {
    return {
      title: this.$store.state.app.pageTitle,
    }
  },
}
</script>
