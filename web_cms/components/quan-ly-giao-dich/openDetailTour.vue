...
export default {
  ...
  methods: {
    ...
    change_status(value) {
      if (!this.$isNullOrEmpty(this.tour.master.order_status)) {
        const DATA = {
          items: [
            {
              transaction_id: this.tourMaster.id,
    ...
    get_list() {
      if (!this.$isNullOrEmpty(this.tour.tour)) {
        this.items = this.tour.change_history
        this.start_time = moment(
          this.tour.tour.departure_date,
          'DD/MM/YYYY hh:mm:ss'
        ).format('DD/MM/YYYY')
        this.tourMaster = this.tour.master
        this.tourDetail = this.tour.tour
        this.tourInfor = this.tour.tourInfo
        this.status = this.tour.master.order_status
        if (this.status === 0) {
