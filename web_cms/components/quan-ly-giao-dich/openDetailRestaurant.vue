...
export default {
  ...
  methods: {
    ...
    get_list() {
      if (!this.$isNullOrEmpty(this.restaurant.restaurant)) {
        this.items = this.restaurant.change_history
        this.restaurantMaster = this.restaurant.master
        this.restaurantDetail = this.restaurant.restaurant
        this.restaurantInfo = this.restaurant.restaurantInfo
        this.booking_date = moment(
          this.restaurant.restaurant.booking_date,
          'YYYY-MM-DD'
        ).format('DD/MM/YYYY')
        this.booking =
          this.restaurant.restaurant.booking_hour + ' - ' + this.booking_date
        this.status = this.restaurant.master.order_status
        if (this.status === 0) {
    ...
    change_status() {
      if (!this.$isNullOrEmpty(this.restaurant.master.order_status)) {
        const DATA = {
          items: [
            {
              transaction_id: this.restaurantMaster.id,
