<template>
  <div class="edit_main_evaluate" style="background-color: #f4f7fe">
    <div class="pb-14" style="background-color: #fff">
      <!-- // chức năng  -->
        ...
        <v-col cols="1" class="py-0">
          <div class="d-flex">
            <div v-for="(images, index) in commentImages" :key="index">
              <div class="mr-3">
                <v-img max-width="120px" max-height="100px" :src="images.url">
                </v-img>
              </div>
            </div>
          </div>
    ...
    <yes-no-alert
      :open="open_reset"
      alert-msg="Bạn có chắc chắn muốn nhập lại câu trả lời đánh giá?"
    ...
    <yes-no-alert
      :open="open_update"
      alert-msg="Bạn có chắc chắn muốn cập nhật đánh giá?"
    ...
    <yes-no-alert
      :open="open_changes"
      alert-msg="Bạn có chắc chắn muốn thay đổi trạng thái lịch sử không?"
...
export default {
  ...
  methods: {
    ...
    changes_status() {
      if ((this.changes_item_status || []).length !== 0) {
        const DATA = {
          updateStatusCommentRequestList: [
            {
              id: this.changes_item_status.id,
              type: this.changes_item_status.comment_type,
              status: this.changes_item_status.status,
            },
          ],
        }
        ...
        this.$store
          ...
          .then((res) => {
            if (!res.error) {
              this.$showSuccess(
                this,
                'Thay đổi trạng thái lịch sử Đánh giá  thành công'
              )
            }
          })
          .finally(() => {
    ...
    update() {
      const data = {
        id: this.$store.state.khach_hang.customerId.id,
        type: this.data.comment_type,
      ...
      this.$store
        ...
        .then((response) => {
          if (!response.error) {
            this.$emit('exit')
            this.$showSuccess(this, 'Cập nhật đánh giá thành công')
            this.$emit('toggle')
            this.$store.commit(
              'app/setTitle',
              `Đánh giá - ${this.$store.state.khach_hang.customerId.info.name}`
            )
          }
        })
        .catch((e) => {
