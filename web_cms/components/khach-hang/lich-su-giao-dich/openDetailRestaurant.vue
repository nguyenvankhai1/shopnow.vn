<template>
  <div class="main_detail_restaurant">
      ...
      <v-row class="pl-4">
        <v-col cols="7" class="mr-5" style="background-color: #fff">
          <v-row class="px-3">
            ...
            >
              ...
              <v-card-text class="pt-0" style="color: #3f69b8"
                >#
                {{
                  $isNullOrEmpty(restaurantMaster)
                    ? ''
                    : restaurantMaster.order_id_mask
                }}
              </v-card-text>
            </v-col>
            <v-col
          ...
          <v-card-text class="fs-17 font-weight-bold"
            >Thông tin đặt bàn</v-card-text
          >
          <v-row>
            <v-col cols="4">
              <v-card-text class="pt-0">Thời gian đặt:</v-card-text>
              <v-card-text class="pt-0">Số lượng khách:</v-card-text>
            </v-col>
            <v-col cols="8" style="text-align: right">
              <v-card-text class="pt-0">{{
                $isNullOrEmpty(booking) ? '' : booking
              }}</v-card-text>
              <v-card-text class="pt-0">{{
                $isNullOrEmpty(restaurantDetail)
                  ? ''
                  : restaurantDetail.cust_quantity
              }}</v-card-text>
            </v-col>
          </v-row>
          <v-row class="px-4">
          ...
          <v-row>
            ...
            <v-col cols="8" style="text-align: right">
              <v-card-text class="pt-0">{{
                $isNullOrEmpty(restaurantMaster)
                  ? ''
                  : restaurantMaster.cust_name
              }}</v-card-text>
              <v-card-text class="pt-0">{{
                $isNullOrEmpty(restaurantMaster)
                  ? ''
                  : restaurantMaster.cust_phone
              }}</v-card-text>
              <v-card-text class="pt-0" style="height: 38px">{{
                $isNullOrEmpty(restaurantMaster)
                  ? ''
                  : restaurantMaster.nation_code
              }}</v-card-text>
              <v-card-text class="pt-0">{{
                $isNullOrEmpty(restaurantMaster)
                  ? ''
                  : restaurantMaster.cust_email
              }}</v-card-text>
              <v-card-text class="pt-0">{{
                $isNullOrEmpty(restaurantMaster)
                  ? ''
              ...
              <v-card-text class="pt-0">{{
                $isNullOrEmpty(restaurantMaster) ? '' : restaurantMaster.note
              }}</v-card-text>
            </v-col>
          </v-row>
          <v-row class="px-4">
        ...
        <v-col cols="4" style="background-color: #fff">
          ...
          <v-row>
            <v-col cols="11" class="pt-0 ml-4">
              <v-img
                :src="`${
                  $isNullOrEmpty(restaurantInfo) ? '' : restaurantInfo.thumbnail
                }`"
              >
              </v-img>
            </v-col>
          ...
          <v-row>
            <v-col cols="12" class="py-0 ml-4">
              <div class="fs-17 font-weight-bold mb-2">
                {{ $isNullOrEmpty(restaurantInfo) ? '' : restaurantInfo.name }}
              </div>
              <div class="fs-14" style="color: #3f69b8">
                {{
                  $isNullOrEmpty(restaurantInfo) ? '' : restaurantInfo.address
                }}
              </div>
            </v-col>
          </v-row>
...
export default {
  ...
  methods: {
    ...
    get_list() {
      const data = {
        transaction_id: this.restaurant.id,
        transaction_type: this.restaurant.product_type,
      }
      this.$store.dispatch('giao_dich/detail', data).then((response) => {
        if (!response.error) {
          this.items = response.data.data.change_history
          this.restaurantMaster = response.data.data.master
          this.restaurantDetail = response.data.data.restaurant
          this.restaurantInfo = response.data.data.restaurantInfo
          this.booking_date = moment(
            response.data.data.restaurant.booking_date,
            'YYYY-MM-DD'
          ).format('DD/MM/YYYY')
          this.booking =
            response.data.data.restaurant.booking_hour +
            ' - ' +
            this.booking_date
    ...
    set_status(status) {
      ...
      this.statusDefault = this.restaurantMaster.order_status
    },
    ...
    change_status_history(description) {
      this.description = description
      this.status = this.status_new
    },
    change_status() {
      if (!this.$isNullOrEmpty(this.restaurant.order_status)) {
        const DATA = {
          items: [
            {
              transaction_id: this.restaurant.id,
