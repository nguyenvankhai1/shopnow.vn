            ...
            >
              <v-card-text
                ...
                >{{
                  $isNullOrEmpty(hotelDetail)
                    ? ''
                    : `${Intl.NumberFormat().format(hotelDetail.total_amount)}`
                }}
                {{
                  $isNullOrEmpty(hotelDetail) ? '' : hotelDetail.currency_code
                }}
...
export default {
  ...
  methods: {
    ...
    change_status() {
      if (!this.$isNullOrEmpty(this.hotel.order_status)) {
        const DATA = {
          items: [
            {
              transaction_id: this.hotel.id,
    ...
    get_list() {
      const data = {
        transaction_id: this.hotel.id,
        transaction_type: this.hotel.product_type,
      }
      this.$store.dispatch('giao_dich/detail', data).then((response) => {
        if (!response.error) {
          if (!this.$isNullOrEmpty(response.data.data.hotelInfo)) {
            this.items = response.data.data.change_history
            this.check_in =
              response.data.data.hotelInfo.checkin_hour +
              ' - ' +
              moment(
                response.data.data.hotel.check_in,
                'DD/MM/YYYY hh:mm:ss'
              ).format('DD/MM/YYYY')
            this.check_out =
              response.data.data.hotelInfo.checkout_hour +
              ' - ' +
              moment(
                response.data.data.hotel.check_out,
                'DD/MM/YYYY hh:mm:ss'
              ).format('DD/MM/YYYY')
            ...
            this.hotelDefault = response.data.data.hotel
            this.hotelDetail = response.data.data.master
            this.hotelInfo = response.data.data.hotelInfo
            this.capacity = response.data.data.hotelInfo.capacity
            this.quantity = response.data.data.hotel.quantity
            this.total_customer = this.capacity * this.quantity
