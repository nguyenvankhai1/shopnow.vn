    ...
    <div>
        ...
        <v-col cols="6" class="py-0">
          ...
          >
            ...
            <v-text-field
              ...
              solo
              dense
              @change="checkRoom($event) ? rooms : (rooms = 1)"
            ></v-text-field>
            <v-btn
              small
              depressed
      ...
      <v-row class="pl-8 mb-8">
        ...
        <v-col cols="6" class="py-0">
          ...
          >
          <v-card-text
            v-if="currency === list_currencies[0] && pricePayment.length > 0"
            class="pl-0 pt-2 font-weight-bold"
            ><span
              >{{
                $formatMoney({
  ...
  watch: {
    ...
    endDate(val) {
      if (this.oldDate > this.endDate) {
        ...
        this.startDateError = ['Thời gian đặt lớn hơn thời gian đến']
      } else {
        this.startDateError = []
  ...
  methods: {
    ...
    validateNumber(value) {
      ...
      if (!this.isNumber(value)) {
        this.required = true
        this.personError = 'Bạn phải nhập là số'
      }
    },
    isNumber(number) {
      const str = number + ''
