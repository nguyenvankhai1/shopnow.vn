  ...
  <div class="product-in-category">
    <v-row>
      <v-col>
        <v-text-field
          v-model="search"
          placeholder="Tìm kiếm sku, tên sản phẩm, trạng thái, nhà cung cấp"
    ...
    get_search() {
      ...
      if (
        ...
        for (let i = 0; i < this.items_default.length; i++) {
          ...
          )} ${this.$removeAccents(this.items_default[i].enterprise_name)}${
            this.items_default[i].status === 1 ? 'hien thi' : 'an'
          }`
