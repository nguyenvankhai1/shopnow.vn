  ...
  <div class="tourism_product pb-12">
    <div>
      ...
      <v-row class="pl-8 mb-2">
        ...
        <v-col cols="9" class="py-0 ml-1">
          <v-card-text class="pl-0 pt-0 font-weight-bold">
            {{
              $formatMoney({
                amount: $isNullOrEmpty(priceProduct.price)
                  ? 0
                  : priceProduct.price,
              })
            }}
      ...
      <v-row class="pl-8 mt-6 mb-4">
        <v-col cols="2" class="pl-10 py-0">
          <v-card-text class="text-subtitle-2 pt-0">Thanh toán</v-card-text>
        </v-col>
        <v-col cols="9" class="py-0 ml-1">
          <v-card-text
            v-if="currency === list_currencies[0]"
            class="pl-0 pt-0 font-weight-bold"
            ><span
              >{{
                $isNullOrEmpty(total_price)
                  ? ''
      ...
      <v-row class="pl-8">
        <v-col cols="2" class="pl-10 py-0 pr-0">
          <v-card-text class="text-subtitle-2 pt-2 pr-0"
      ...
      <v-row class="pl-8">
        <v-col cols="2" class="pl-10 py-0 pr-0">
          <v-card-text class="text-subtitle-2 pt-2 pr-0"
            >Phương thức thanh toán <span class="red--text">*</span>
          </v-card-text>
        </v-col>
        <v-col cols="9" class="py-0">
          <v-select
  ...
  methods: {
    ...
    total_price_products() {
      if (!this.$isNullOrEmpty(this.product)) {
        this.total_price = this.priceProduct.price * this.total_product
      }
    },
    change_products() {
      this.$store.dispatch('giao_dich/tourism_product').then((response) => {
        if (!response.error) {
          if ((response.data.data || []).length !== 0) {
            for (let i = 0; i < response.data.data.length; i++) {
              this.priceProduct = response.data.data[i]
      ...
      if (!this.$isNullOrEmpty(this.currency)) {
        const data = {
          ...
          quantity: !this.$isNullOrEmpty(this.total_product)
            ? this.total_product
            : '',
          productId: !this.$isNullOrEmpty(this.product) ? this.product : '',
        }
        this.$store
