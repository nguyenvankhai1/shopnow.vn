  ...
  <div>
    <client-only>
      ...
      <v-tabs-items v-model="tabmenu" style="background-color: #f5f7fe">
        <v-tab-item>
          <div class="add-product-tour-info">
                  ...
                  <v-tabs-items v-model="tab" style="background-color: #f5f7fe">
                    ...
                    >
                      <v-text-field
                        ...
                        :outlined="language.restaurant_msg.length !== 0"
                  ...
                  >
                    ...
                    >
                      <quill-editor
                        ...
                        :options="
                          $store.state.nha_cung_cap_san_pham.editorOption
                        "
                        class="quill-editor pb-12"
                      />
                    </v-tab-item>
...
import Suggestions from '~/components/product-nha-cung-cap/tour/suggestions/index'
import Schedule from '~/components/product-nha-cung-cap/tour/schedule/index'
import DepartureSchedule from '~/components/product-nha-cung-cap/tour/DepartureSchedule/index'
...
export default {
  ...
  created() {
    ...
    this.$store.dispatch('nha_cung_cap_san_pham/tour_gen_sku').then((res) => {
      if (!res.error) {
  ...
  methods: {
    ...
    set_vehicle() {
      this.$store.dispatch('nha_cung_cap_san_pham/tour_vehicle').then((res) => {
        if (!res.error) {
    ...
    // upload_suggestions_image(images) {
    //   let suggestions = []
    ...
    //           }
    //           suggestions = [...arr]
    //           this.insert(images, suggestions)
    //         }
    //       }
    //     })
    // },
    insert(tourThumbnailList) {
      ...
      this.$store.dispatch('product/tour_add', DATA).then((res) => {
        if (!res.error) {
          this.$showSuccess(this, 'Thêm sản phẩm tour thành công')
          // this.$emit('save')
          this.$emit('success')
          this.$emit('toggle1')
        }
