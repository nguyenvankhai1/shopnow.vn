  ...
  <div>
    <client-only>
      ...
      <v-tabs-items v-model="tabmenu" style="background-color: #f5f7fe">
        <v-tab-item>
          <div class="add-product-restaurant-info">
                  ...
                  <div>
                    ...
                    <div class="d-flex">
                      <v-text-field
                        ...
                        dense
                        hide-details
                        @keydown.enter="set_address_to_map"
                      >
                      </v-text-field>
                      <v-btn
                        color="white"
                  ...
                  <v-row>
                    ...
                    <v-col cols="4" class="py-0">
                      ...
                      <v-autocomplete
                        ...
                        :outlined="type_dish_msg.length !== 0"
                        :readonly="uncorrected"
                        class="border-1 fs-14"
                        solo
                        flat
                ...
                <v-col class="pb-0" cols="11">
                  ...
                  >
                    ...
                    >
                      <!--                      <quill-editor-->
                      <!--                        ref="myTextEditor"-->
                      <!--                        v-model="language.menu"-->
                      <!--                        :options="$store.state.product.editorOption"-->
                      <!--                        class="quill-editor pb-12"-->
                      <!--                        :disabled="uncorrected"-->
                      <!--                        @input="language.menu_msg = []"-->
                      <!--                      />-->
                      <vue-editor
                        v-model="language.menu"
                        :disabled="uncorrected"
                        use-custom-image-handler
                        @text-change="language.menu_msg = []"
                      />
                      <div
    ...
    <yes-no-alert
      :open="open_save"
      alert-msg="Bạn chắc chắn muốn lưu thông tin được thay đổi không?"
...
import Suggestions from '~/components/product-nha-cung-cap/edit/restaurant/suggestions/index'
import Services from '~/components/product-nha-cung-cap/edit/restaurant/services/index'
...
export default {
  ...
  methods: {
    ...
    reset_data() {
      ...
      this.hidden = 1
      this.center = { lat: 21.028511, lng: 105.804817 }
      this.lat = 21.028511
      this.lng = 105.804817
      this.zoom = 15
      this.category = [2]
    ...
    set_data() {
      ...
      // const suggestions = []
      // for (let i = 0; i < this.data.restaurantSuggestions.length; i++) {
      //   listsuggestions.push({
      //     hotel_id: this.data.restaurantSuggestions[i].restaurant_id,
      //     id: this.data.restaurantSuggestions[i].id,
      //     name: this.data.restaurantSuggestions[i].name,
      //     img: this.data.restaurantSuggestions[i].url_image,
      //     file: '',
      //     url: this.data.restaurantSuggestions[i].url,
      //     status: this.data.restaurantSuggestions[i].status,
      //   })
      //   suggestions.push({
      //     name: this.data.restaurantSuggestions[i].name,
      //     image_url: this.data.restaurantSuggestions[i].url_image,
      //     target_url: this.data.restaurantSuggestions[i].url,
      //     status: this.data.restaurantSuggestions[i].status,
      //   })
      // }
    ...
    setMarker(value) {
      ...
      // }
      // this.set_address_to_map(1)
    },
    set_address_to_map(check) {
      const DATA = {
    ...
    //   const folder = 'restaurant-product-suggestions-img'
    //   const formData = new FormData()
    //   const index = []
    //   for (let i = 0; i < this.listsuggestions.length; i++) {
    //     if (!this.$isNullOrEmpty(this.listsuggestions[i].file)) {
    ...
    insert() {
      ...
      this.$store
        .dispatch('nha_cung_cap_san_pham/restaurant_edit', DATA)
        .then((res) => {
          if (!res.error) {
            this.$showSuccess(this, 'Cập nhật sản phẩm nhà hàng thành công')
            this.$emit('toggle')
            this.$emit('success')
          }
        })
    ...
    get_review() {
      ...
      this.$store
        .dispatch('nha_cung_cap_san_pham/review_restaurant', DATA)
        .then((res) => {
          if (!res.error) {
