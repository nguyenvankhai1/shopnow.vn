<template>
  <v-col class="py-0" cols="11">
    ...
    <v-row>
      ...
      <v-col v-if="!readonly" cols="auto" class="pl-0 pr-5">
        <v-btn
          color="#47BBFF"
...
export default {
  ...
  created() {
    this.$store
      .dispatch('nha_cung_cap_san_pham/get_list_suggestion_in', {
      ...
      .then((res) => {
        if (!res.error) {
          ...
          this.items = this.items_default = res.data.data
          this.$emit('set_data', res.data.data)
        }
      })
  },
  methods: {
