...
import Add from '~/components/Product/edit/hotel/room/add'
import Edit from '~/components/Product/edit/hotel/room/edit'
...
export default {
  ...
  methods: {
    ...
    confirm_delete() {
      this.$store
        .dispatch('product/room_delete', [this.data_row.id])
        .then((res) => {
          if (!res.error) {
    ...
    confirm_deletes() {
      ...
      this.$store
        .dispatch('product/room_delete', [...arrDelete])
        .then((res) => {
          if (!res.error) {
            this.items = [...arr]
    ...
    update_status() {
      ...
      this.$store.dispatch('product/room_edit', DATA).then((res) => {
        if (!res.error) {
          const arr = [...this.items]
    ...
    change_sort(object) {
      ...
      this.$store.dispatch('product/room_sort', DATA).then((res) => {
        if (!res.error) {
          this.items = [...arr]
        }
      })
