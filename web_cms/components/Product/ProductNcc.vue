<template>
  <div class="product-ncc">
    ...
    <v-data-table
      ...
      :loading="$wait.is('loadingProductNcc')"
      item-key="stt"
      show-select
      fixed-header
      hide-default-footer
...
export default {
  ...
  methods: {
    ...
    get_list() {
      this.$wait.start('loadingProductNcc')

      const DATA = {
        page_index: this.page,
        page_size: this.pageSize,
        search: (this.sku || []).length !== 0 ? this.sku.trim() : null,
        type: this.$store.state.app.type.ncc, // sản phẩm của ncc tạo
        sku:
      ...
      this.$store
        ...
        .finally(() => {
          this.$wait.end('loadingProductNcc')
        })

      // console.log(
      //   'data selected: ',
      //   this.data_selected,
      //   this.data_selected.length
      // )
      this.open_search = false
    },
    ...
    change_status() {
      if (!this.$isNullOrEmpty(this.status_data)) {
        ...
        this.$store
          .dispatch('product/change_approval_status', DATA)
          .then((res) => {
            if (!res.error) {
