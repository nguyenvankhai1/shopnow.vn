<template>
  <div style="background-color: #f4f7fe">
    ...
    <yes-no-alert
      :open="open_update"
      alert-msg="Bạn chắc chắn muốn lưu thông tin menu không?"
  ...
  watch: {
    open(value) {
      if (value === 3) {
        ...
        this.nameMenu = this.data.name
        const arr = []
        for (let i = 0; i < this.data.menuSites.length; i++) {
          arr.push(this.data.menuSites[i].sites_id)
        }
        this.sites = [...arr]
        this.treeData = this.data.detailMenu
        this.getSites()
      }
    },
  },
  ...
  methods: {
    ...
    update() {
      ...
      this.$store
        ...
        .then((response) => {
          if (!response.error) {
            this.$showSuccess(this, 'Cập nhật menu thành công')
            this.$emit('toggle')
            this.$emit('OK')
          }
        })
