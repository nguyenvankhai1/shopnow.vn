...
export default {
  ...
  watch: {
    open(value) {
      if (value) {
        if (this.listisreset === true) {
          this.get_list_post_all()
        } else {
          this.get_list()
        }
  ...
  methods: {
    get_list() {
      const DATA = {
        category_id: this.info.categoryPost.id,
        pageIdx: this.page,
        pageSize: this.pageSize,
        search: (this.search || []).length !== 0 ? this.search.trim() : '',
      }

      this.$store.dispatch('danh_muc_bai_viet/out_post', DATA).then((res) => {
        this.items = []
    ...
    get_list_post_all() {
      ...
      this.$store.dispatch('bai_viet/list', DATA).then((res) => {
        this.items = []
